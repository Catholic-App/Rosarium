...
// --- ROSARIUM: registro de terços (adicionado pelo assistente) ---
function salvarRegistroTerco(tipoTerco, dateISO) {
  try {
    const key = 'rosarium_calendar_entries';
    const entries = JSON.parse(localStorage.getItem(key) || '[]');
    const date = dateISO || (new Date()).toISOString().slice(0,10); // YYYY-MM-DD
    entries.push({ prayerId: tipoTerco, date: date, ts: Date.now() });
    localStorage.setItem(key, JSON.stringify(entries));
    console.log('Registro salvo', tipoTerco, date);
    return true;
  } catch (e) {
...
// helper to carregar entradas
function carregarEntradasCalendario() {
  const key = 'rosarium_calendar_entries';
  const entries = JSON.parse(localStorage.getItem(key) || '[]');
  const byDate = entries.reduce((acc, it) => {
