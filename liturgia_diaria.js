...
function displayLiturgia(date) {
    ...
    liturgia.leituras.forEach(leitura => {
        ...
        `;
         if (leitura.titulo.includes('Evangelho')) {
	            ...
	            // O texto do Evangelho é o último a ser processado, então ele deve ser o que fica.
    ...
    // Armazenar o texto do Evangelho para a Lectio Divina
	    ...
	    if (evangelhoTexto && evangelhoReferencia) {
	        ...
	        // Se não houver Evangelho, armazena strings padrão para evitar o erro de "Referência não disponível"
	        localStorage.setItem('lectio_evangelho_texto', 'Texto do Evangelho não disponível.');
	        localStorage.setItem('lectio_evangelho_referencia', 'Referência não disponível.');
	    } loadLiturgiaData();
    ...
    if (prevDayButton && nextDayButton && dateDisplay) {
        ...
        // Disparar evento para que outros scripts saibam que a liturgia foi carregada
        document.dispatchEvent(new Event('liturgiaLoaded'));
    }
});
