...
function displayLiturgia(date) {
    ...
    // Adicionar Evangelho de campos de nível superior (para dias que não estão em 'leituras')
    if (liturgia.evangelho && liturgia.evangelho_referencia) {
        // Prioriza o Evangelho de nível superior se não foi encontrado nas leituras
        if (!evangelhoTexto) {
            evangelhoTexto = liturgia.evangelho.replace(/\\n/g, '<br>');
            evangelhoReferencia = liturgia.evangelho_referencia;
            
            // Adicionar o Evangelho ao HTML da Liturgia Diária, se ainda não estiver lá
            html += `
                <div class="leitura-item">
                    <h3 class="leitura-titulo">${liturgia.evangelho_titulo || 'Evangelho'} (${liturgia.evangelho_referencia})</h3>
                    <div class="leitura-texto">${evangelhoTexto}</div>
                </div>
            `;
        }
    }
