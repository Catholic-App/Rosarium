...
async function loadSantosData() {
    try {
        ...
        const dataResponse = await fetch('/home/ubuntu/santos_data.json');
        santosData = await dataResponse.json();

        // Carregar lista de santos por mês
        const listaResponse = await fetch('/home/ubuntu/santos_lista.json');
...
function displaySantoDoDia(date) {
    ...
    if (santo) {
        container.innerHTML = `
            <h2 class="page-title">${santo.nome}</h2>
        ...
        container.innerHTML = `
            <h2 class="page-title">Santo do Dia</h2>
            <div class="card-content">
