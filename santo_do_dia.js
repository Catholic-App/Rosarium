...
// Função para obter o(s) Santo(s) do Dia
function getSantoDoDia(date) {
    const month = String(date.getMonth() + 1).padStart(2, '0');
    const day = String(date.getDate()).padStart(2, '0');
    const key = `${month}-${day}`;
    
    const data = santosData[key];
    
    // Se for array, retorna o array (caso de múltiplos santos)
    if (Array.isArray(data)) {
        return data;
    }
    
    // Se for objeto único, retorna como array de um elemento
    if (data && typeof data === 'object') {
        // Se o objeto tiver a chave 'biografia', é um santo único
        if (data.biografia) {
            return [data];
        }
        // Caso contrário, pode ser um objeto com múltiplos santos aninhados
        // Vamos assumir que se não for um array, é um objeto único.
        return [data];
    }
...
function displaySantoDoDia(date) {
    const santos = getSantoDoDia(date);
    ...
    if (santos && santos.length > 0) {
        ...
        if (santos.length === 1) {
            ...
            pageTitle.textContent = 'Santos do Dia';
        }
        ...
        // Exibir cada santo
        santos.forEach((santo, index) => {
            // Se houver mais de um santo, adicionar separador visual
            if (index > 0) {
                html += `<hr style="margin: 2rem 0; border: none; border-top: 1px solid var(--primary);">`;
            }
            
            // Garantir que o título seja o nome se não houver título específico
            const titulo = santo.titulo || santo.nome;
            
            html += `
                <div class="santo-item" style="margin-bottom: ${index < santos.length - 1 ? '0' : '1rem'};">
                    <h4 class="card-title" style="margin-bottom: 0.75rem; color: var(--primary);">${titulo}</h4>
                    <div style="margin-bottom: 1rem;">
                        <p style="line-height: 1.6;">${santo.biografia}</p>
                    </div>
                    <p style="margin-top: 1rem; font-style: italic; color: var(--muted-foreground);">Cor Litúrgica: ${santo.cor || 'Não Informada'}</p>
                </div>
            `;
        });
