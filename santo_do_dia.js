...
function displaySantoDoDia(date) {
    ...
    if (santo) {
        // Atualiza o título da página para o nome do Santo
        pageTitle.textContent = santo.nome;
        ...
        container.innerHTML = `
            <div class="santo-header-card" style="text-align: center; margin-bottom: 1.5rem;">
                ...
                <h3 class="card-title" style="margin-bottom: 0.25rem;">${santo.nome}</h3>
                <p class="text-sm text-muted-foreground">${dateText}</p>
            </div>

            <div class="card-content">
                <h4 class="card-title" style="margin-top: 1rem;">Oração:</h4>
                <p class="text-italic">Rogai por nós, ${santo.nome}, intercedei a Deus por nós.</p>
